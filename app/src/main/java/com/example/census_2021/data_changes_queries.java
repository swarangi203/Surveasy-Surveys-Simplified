...
public class data_changes_queries extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_data_changes_queries);
        recyclerView=findViewById(R.id.mainlayout);
        reference3=FirebaseDatabase.getInstance().getReference("Queries").child("Data_name_change_query");
                        ...
                        if(dataSnapshot.exists())
                        {
                            ...
                            reference2.addListenerForSingleValueEvent(new ValueEventListener() {
                                ...
                                public void onDataChange(@NonNull DataSnapshot snapshot) {
                                    ...
                                    {
                                        // Toast.makeText(data_changes_queries.this, dataSnapshot1.getValue().toString(), Toast.LENGTH_SHORT).show();
                                                    ...
                                                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                                                        ...
                                                        for(DataSnapshot dataSnapshot2 : snapshot.getChildren())
                                                        {
                                                            if(dataSnapshot2.exists()) {
                                                                if (dataSnapshot2.getKey().toString().equals("name")) {
                                                                    DatabaseReference reference1 = dataSnapshot2.getRef();
                                                                    reference1.setValue(value.getText().toString());
                                                                    Toast.makeText(data_changes_queries.this, "Name Changed Successfully", Toast.LENGTH_SHORT).show();
                                                                    reference3.child(uid).removeValue();
